...
function checkEmail() {
  ...
  if (!inputEmail.value) {
    ...
    // Check validation
    !inputEmail.value.match(/([A-z0-9.][^@])+@([A-z0-9-])+(?:\.[A-z]+)$/g)
  ) {
    ...
    let imgHeading = `
        ...
        <div class="showImages-${extractUsername(
          inputEmail.value
        )}" id="showImages"></div>
        </div>`;
...
function extractUsername(email) {
  ...
  //   get user's first part of email for name
...
function viewImages(arr, username) {
  ...
  elem.innerHTML = `<div class="showImages" id="showImages">${convertURL(
    arr
  )}</div>`;
}
